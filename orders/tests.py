...
class OrdersModelTests(TestCase):
	...
	def test_order_item_total_price(self):
		...
		item = OrderItem.objects.create(order=o, product_title="Art", unit_price=Decimal("12.50"), quantity=2)
		self.assertEqual(item.total_price, Decimal("25.00"))

	def test_reservation_default_expiry_hours(self):
		user = User.objects.create_user(username="tester", password="pass")
		r = Reservation.create_reservation(user=user, product_title="P", product_sku="SKU-1", quantity=1)
...
class PaymentsHelperTests(TestCase):
	...
	def test_create_stripe_payment_intent_fallback_and_real_flow(self):
		...
		# Temporarily ensure no real stripe module is used by inserting a dummy module that raises
		real_stripe = sys.modules.get("stripe")
		try:
			# Create a fake stripe module whose PaymentIntent.create returns a fake client_secret
			fake_stripe = types.SimpleNamespace()
			payment_intent_ns = types.SimpleNamespace()

			def fake_create(*args, **kwargs):
				return types.SimpleNamespace(client_secret="fake_cs_123")

			payment_intent_ns.create = fake_create
			fake_stripe.PaymentIntent = payment_intent_ns
			sys.modules["stripe"] = fake_stripe

			cs = payments.create_stripe_payment_intent(amount=100, currency="eur")
			self.assertEqual(cs, "fake_cs_123")
		finally:
		...
		def test_issue_refund_idempotent_model_helper(self):
			...
			pr = PaymentRecord.objects.create(order=order, provider="stripe", provider_payment_id="ch_999", amount=Decimal("40.00"), currency="EUR", status=PaymentRecord.STATUS_SUCCEEDED)

			# Patch stripe.Refund.create and ensure it's called only once across two helper invocations
			calls = []
			def fake_refund(**k):
				calls.append(k)
			...
			fake_stripe = types.SimpleNamespace()
			fake_stripe.Refund = types.SimpleNamespace(create=fake_refund)
			sys.modules["stripe"] = fake_stripe
			...
			# Second call should be idempotent and not call stripe.Refund.create again
...
class ApiAndWebhookTests(TestCase):
	...
	def test_admin_action_mark_shipped_updates_orders(self):
		...
		staff = User.objects.create_user(username="admin", password="pw", is_staff=True, is_superuser=True)
		c = Client()
		c.force_login(staff)
		...
		resp = c.post(url, {'action': 'mark_shipped', '_selected_action': [order1.pk, order2.pk]})
	...
	def test_admin_action_mark_refunded_issues_refunds_and_marks(self):
		...
		from django.test import Client
		staff = User.objects.create_user(username="admin2", password="pw", is_staff=True, is_superuser=True)
		c = Client()
		c.force_login(staff)
		order = Order.objects.create(total=Decimal("30.00"))
		pr = PaymentRecord.objects.create(order=order, provider="stripe", provider_payment_id="ch_abc", amount=Decimal("30.00"), currency="EUR", status=PaymentRecord.STATUS_SUCCEEDED)

		# patch PaymentRecord.issue_refund to simulate success
		with mock.patch.object(PaymentRecord, 'issue_refund', return_value={'id': 're_ok'}):
			url = reverse('admin:orders_order_changelist')
			resp = c.post(url, {'action': 'mark_refunded', '_selected_action': [order.pk]})
			self.assertIn(resp.status_code, (200, 302))
	...
	def test_create_order_requires_items_and_guest_email_for_anonymous(self):
		...
		payload = {"items": [{"product_title": "A", "unit_price": "10.00", "quantity": 1}]}
	...
	@mock.patch("orders.payments.create_stripe_payment_intent", return_value="cs_test_123")
	def test_start_payment_view_creates_payment_record(self, _mock_intent):
		...
		self.assertEqual(pr.provider, "stripe")
		self.assertEqual(pr.amount, order.total)

	def test_start_payment_idempotency_returns_same_payment_and_client_secret(self):
		order = Order.objects.create(total=Decimal("50.00"))
		url = reverse("orders-start-payment", args=[order.pk])
		# send Idempotency-Key via header; mock the provider helper to return a known client_secret
		with mock.patch("orders.payments.create_stripe_payment_intent", return_value="cs_idempotent_123"):
		...
		data1 = resp1.json()
		data2 = resp2.json()
		self.assertEqual(data1.get("client_secret"), data2.get("client_secret"))

		# Only one PaymentRecord for the order with that idempotency key
		prs = PaymentRecord.objects.filter(order=order, idempotency_key="startpay-1")
	...
	def test_stripe_webhook_payment_succeeded_updates_order(self):
		...
		fake_event = {
			...
			"data": {"object": {"id": "pi_123", "metadata": {"order_id": order.pk}}},
		}

		with mock.patch("orders.webhooks.verify_stripe_event", return_value=fake_event):
			resp = self.client.post(reverse("orders-webhook"), data=b"{}", content_type="application/json", HTTP_STRIPE_SIGNATURE="sig")
			self.assertEqual(resp.status_code, 200)
			order.refresh_from_db()
	...
	def test_stock_decrements_on_webhook_success_and_flags_shortage(self):
		...
		# Create an order with two items: 3x SKU-A and 2x SKU-B (SKU-B will be short)
		order = Order.objects.create(total=Decimal("0.00"))
		OrderItem.objects.create(order=order, product_title="A", product_sku="SKU-A", unit_price=Decimal("1.00"), quantity=3)
		OrderItem.objects.create(order=order, product_title="B", product_sku="SKU-B", unit_price=Decimal("1.00"), quantity=2)

		fake_event = {
			"id": "evt_stock_1",
			"type": "payment_intent.succeeded",
			"data": {"object": {"id": "pi_stock", "metadata": {"order_id": order.pk}}},
		}
		...
		b = StockItem.objects.get(sku="SKU-B")

		# SKU-A had 5, ordered 3 -> should be 2
		self.assertEqual(a.stock, 2)
		# SKU-B had 1, ordered 2 -> cannot fulfill; stock may remain 1 or be zero depending on code path, but order should be flagged
	...
	def test_webhook_sends_confirmation_email_when_recipient_present(self):
		...
		order = Order.objects.create(total=Decimal("12.00"), guest_email="guest@example.com")

		fake_event = {
			"id": "evt_email_1",
			"type": "payment_intent.succeeded",
			"data": {"object": {"id": "pi_email", "metadata": {"order_id": order.pk}}},
		}
	...
	def test_concurrent_webhook_decrements_do_not_oversell(self):
		# Simulate two separate orders trying to buy the last unit of a SKU at the same time.
		from gallery.models import StockItem
		...
		OrderItem.objects.create(order=order1, product_title="C", product_sku="SKU-C", unit_price=Decimal("5.00"), quantity=1)
		order2 = Order.objects.create(total=Decimal("5.00"))
		OrderItem.objects.create(order=order2, product_title="C", product_sku="SKU-C", unit_price=Decimal("5.00"), quantity=1)

		# Two distinct provider events (so webhook dedupe does not skip the second)
		event1 = {"id": "evt_conc_1", "type": "payment_intent.succeeded", "data": {"object": {"id": "pi_c1", "metadata": {"order_id": order1.pk}}}}
		event2 = {"id": "evt_conc_2", "type": "payment_intent.succeeded", "data": {"object": {"id": "pi_c2", "metadata": {"order_id": order2.pk}}}}

		# Worker posts a single webhook with its own mocked verification
		def worker(event):
			with mock.patch("orders.webhooks.verify_stripe_event", return_value=event):
				c = APIClient()
				c.post(reverse("orders-webhook"), data=b"{}", content_type="application/json", HTTP_STRIPE_SIGNATURE="sig")

		# Start both workers concurrently
		t1 = threading.Thread(target=worker, args=(event1,))
		t2 = threading.Thread(target=worker, args=(event2,))
		t1.start()
		t2.start()
		t1.join()
		t2.join()

		# After both finish, stock for SKU-C must not be negative. On SQLite row locks
		# are not supported the same way as Postgres, so the final stock may be 0
		# (one worker won the race) or 1 (neither decremented in this simulated
		# concurrency). Ensure we don't oversell (stock < 0) and accept either
	...
	def test_order_create_reserves_single_item_and_snapshots_status(self):
		from gallery.models import StockItem
		# single unique painting (stock=1) - mark as unique so reservation uses status transitions
		p = StockItem.objects.create(title="Unique", sku="ONE-1", stock=1, is_unique=True)
		url = reverse("orders-create")
		payload = {"guest_email": "g@x.com", "items": [{"product_title": p.title, "product_sku": p.sku, "unit_price": "100.00", "quantity": 1}]}
		resp = self.client.post(url, data=payload, format="json")
		self.assertEqual(resp.status_code, 201)
		p.refresh_from_db()
		self.assertEqual(p.status, StockItem.STATUS_RESERVED)
		# verify orderitem snapshot
		order_id = resp.json().get("id") or resp.json().get("pk")
		# fallback: find last order
		from .models import OrderItem
		oi = OrderItem.objects.filter(product_sku=p.sku).last()
		self.assertIsNotNone(oi)
		self.assertEqual(oi.product_status, StockItem.STATUS_RESERVED)
	...
	def test_webhook_idempotency_skips_duplicate_events(self):
		...
		fake_event = {
			"id": "evt_dup_1",
			"type": "payment_intent.succeeded",
			"data": {"object": {"id": "pi_dup", "metadata": {"order_id": order.pk}}},
		}

		with mock.patch("orders.webhooks.verify_stripe_event", return_value=fake_event):
			# first delivery processed
			resp1 = self.client.post(reverse("orders-webhook"), data=b"{}", content_type="application/json", HTTP_STRIPE_SIGNATURE="sig")
			self.assertEqual(resp1.status_code, 200)
			order.refresh_from_db()
			self.assertEqual(order.status, Order.STATUS_PROCESSING)
		...
		with mock.patch("orders.webhooks.verify_stripe_event", return_value=fake_event):
			...
			resp2 = self.client.post(reverse("orders-webhook"), data=b"{}", content_type="application/json", HTTP_STRIPE_SIGNATURE="sig")
			self.assertEqual(resp2.status_code, 200)
	...
	def test_refund_endpoint_requires_staff_and_refunds(self):
		...
		order = Order.objects.create(total=Decimal("30.00"))
		pr = PaymentRecord.objects.create(order=order, provider="stripe", provider_payment_id="ch_123", amount=Decimal("30.00"), currency="EUR", status=PaymentRecord.STATUS_SUCCEEDED)
		...
		fake_stripe = types.SimpleNamespace()
		fake_stripe.Refund = types.SimpleNamespace(create=lambda **k: fake_resp)
...
class MiddlewareTests(TestCase):
	...
	def test_middleware_does_not_affect_other_views(self):
		# Use start-payment URL — middleware should not return 415 for this route
		order = Order.objects.create(total=Decimal("10.00"))
		url = reverse("orders-start-payment", args=[order.pk])
		resp = self.client.post(url, data={"foo": "bar"})
		# Could be 200 or 404 depending on provider behavior; ensure it's not 415
		self.assertNotEqual(resp.status_code, 415)

	def test_enabled_via_settings_still_enforces(self):
		# Re-affirm that with ORDERS_JSON_ONLY_VIEWS explicitly set enforcement works
		url = reverse("orders-create")
