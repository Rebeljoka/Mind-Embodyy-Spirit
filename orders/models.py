...
class Address(models.Model):
    """Model for shipping or billing addresses associated with an order."""
    SHIPPING = "shipping"
    BILLING = "billing"
    ...
    def __str__(self):
        return f"{self.address_type.title()} address for order {self.order.id}"
...
class Order(models.Model):
    ...
    def save(self, *args, **kwargs):
        """Save the order, generating order_number if not set."""
    ...
    def items(self):
        """Get the order items."""
        return self.order_items.all()  # type: ignore
...
class OrderItem(models.Model):
    ...
    # Snapshot fields to prevent breakage if product changes or is deleted
    product_title = models.CharField(max_length=255)
    product_sku = models.CharField(max_length=64, blank=True)
    # Snapshot of product status at ordering time
...
class PaymentRecord(models.Model):
    """Model for payment records associated with orders."""
    ...
    # Provider refund id and timestamp for idempotency/audit
    ...
    # When creating provider-side payment intents we store
    # the client_secret so repeated start-payment calls can reuse it.
    provider_client_secret = models.CharField(
        max_length=255,
        blank=True,
        null=True
    )
    # Optional idempotency key to deduplicate start-payment requests
    ...
    def issue_refund(self, amount=None, idempotency_key=None):
        ...
        - If `provider_refund_id` is already set, returns the stored
          `raw_response`.
        - Otherwise calls the provider refund API, stores provider_refund_id,
          raw_response, status and refunded_at.
        ...
        if self.provider != "stripe":
            raise NotImplementedError(
                "Refunds only implemented for stripe in this helper"
            )
        ...
        if amount is not None:
            refund_kwargs["amount"] = str(int(amount * 100))
...
class Reservation(models.Model):
    """Model for item reservations during checkout."""
    # Reservations lock items for a user during checkout
    user = models.ForeignKey(
        settings.AUTH_USER_MODEL,
        on_delete=models.CASCADE,
    ...
    @classmethod
    def create_reservation(cls, user, product_title, product_sku,
                           quantity=1, hours=4, **kwargs):
        # pylint: disable=too-many-arguments,too-many-positional-arguments
        """Create a new reservation with default expiration."""
...
class ProcessedEvent(models.Model):
    """Record processed webhook event ids for idempotency."""
